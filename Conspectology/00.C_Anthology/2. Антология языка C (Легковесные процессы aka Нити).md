...
Выше мы уже описали, спомощью какой функции можно получить идентификатор нашей нити. Однако, справедливости ради, он не так прост, как `pid`. Как мы видели, идентификатор нити имеет специальный тип `pthread_t`, который далеко не всегда является типом данных, который можно привести к `int` или аналогичным.
